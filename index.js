...
module.exports = function(fileName, converter, concatContent) {
    if (!fileName) {
    ...
    function bufferContents(file) {
        ...
        try {
            ...
            if (concatContent === true)
                data = merge(data, JSON.parse(content));
            else
                data[file.relative.substr(0, file.relative.length - 5)] = JSON.parse(file.contents.toString());
        } catch (err) {
            skipConversion = true;
            return this.emit('error',
                new PluginError('gulp-jsonconcat', 'Error parsing JSON: ' + err + ', file: ' + file.path.slice(file.base.length) + ', content:' + content));
        }
    }
    ...
    function endStream() {
        if (firstFile && !skipConversion) {
            ...
            try {
                ...
                return this.emit('error', new PluginError('gulp-jsonconcat', e, {
                    showStack: true
                }));
            }
        } else {
